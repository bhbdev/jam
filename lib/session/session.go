...
import (
	...
	//"github.com/bhbdev/jam/lib/mail"
	"github.com/jam/lib/redis"
	//"github.com/jam/models"
)
...
func LoadOrNew(r *http.Request) *Session {
	...
	if err != nil {
		logger.Debug("session.LoadOrNew: cookie error, new sess: %s", err)
		return sess
	}
	...
	if err != nil {
		logger.Error("session.LoadOrNew: didn't find %s in Redis: %s", key, err)
	}
...
func (s *Session) Save(w http.ResponseWriter) {
	...
	if _, err := uuid.Parse(s.UUID); err != nil {
		logger.Error("Session.Save: got an invalid UUID session_id: %s", err)
		s.UUID = uuid.New().String()
	}
	...
	if err := redis.Set(key, s, config.SessionCookieMaxAge*time.Second); err != nil {
		logger.Error("Session.Save: couldn't write to Redis: %s", err)
	}
...
func Get(r *http.Request) *Session {
	...
	logger.Error("session.Get(): didn't find session in request context!")
	return nil
}
