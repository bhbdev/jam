...
func NewUserService(db *gorm.DB, session *session.Session) *UserService {
	return &UserService{
		Query:   repo.Use(db),
		DB:      db,
		session: session,
...
func (s *UserService) SetPassword(ctx context.Context, id int64, pwd string) error {
	...
	if err != nil {
		logger.Debug("UserService.SetPassword error hashing password", "error", err)
		return err
	}
...
func (s *UserService) Login(w http.ResponseWriter, r *http.Request, user *models.User) (sessionId string, errs map[string]string) {
	...
	u, err := s.User.WithContext(ctx).
		...
		First()
	if err != nil || u == nil {
		logger.Debug("UserService.Login error no such user found", "user", user)
	...
	err = s.session.Create(ctx, sessionId, userSession, config.SessionExpireTime)
	if err != nil {
	...
	err = s.User.WithContext(ctx).Save(u)
	if err != nil {
...
func (s *UserService) Logout(w http.ResponseWriter, r *http.Request) {
	...
	// expire the session
	err = s.session.Expire(ctx, sessionId.Value, 0)
	if err != nil {
